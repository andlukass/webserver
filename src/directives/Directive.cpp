...
{
    while (config[0] && (std::isspace(config[0])))
        config.erase(0, 1);
...
std::string Directive::getFullValue(std::string &config, char closeChar)
{
    ...
    if (nextWord != std::string(1, closeChar))
        throw Exception("Unclosed directive, expecting \"" + std::string(1, closeChar) + "\". At " + this->_name + " directive");
